...
import { pickNextUnusedMajorsShadow, MAJOR_SHADOW_BOARD, MAJOR_SHADOW_TOTAL } from "@/lib/majorShadowBoard";
...
export async function GET(request: Request) {
  ...
  const [slugs, status] = await Promise.all([
    pickNextUnusedMajorsShadow(count),
    pinterestPoolStatus(MAJOR_SHADOW_BOARD, MAJOR_SHADOW_TOTAL),
  ]);
