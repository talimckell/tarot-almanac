...
import { pickNextUnusedCupsGift, CUPS_GIFT_BOARD, CUPS_GIFT_TOTAL } from "@/lib/cupsGiftBoard";
...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(count) || count < 1 || count > CUPS_GIFT_TOTAL) {
    return new Response("Bad request", { status: 400 });
  }

  const [slugs, status] = await Promise.all([
    pickNextUnusedCupsGift(count),
    pinterestPoolStatus(CUPS_GIFT_BOARD, CUPS_GIFT_TOTAL),
  ]);
