...
import { pickNextUnusedPentaclesReclaimed, PENTACLES_RECLAIMED_BOARD, PENTACLES_RECLAIMED_TOTAL } from "@/lib/pentaclesReclaimedBoard";
...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(count) || count < 1 || count > PENTACLES_RECLAIMED_TOTAL) {
    return new Response("Bad request", { status: 400 });
  }

  const [slugs, status] = await Promise.all([
    pickNextUnusedPentaclesReclaimed(count),
    pinterestPoolStatus(PENTACLES_RECLAIMED_BOARD, PENTACLES_RECLAIMED_TOTAL),
  ]);
