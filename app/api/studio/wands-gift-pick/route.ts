...
import { pickNextUnusedWandsGift, WANDS_GIFT_BOARD, WANDS_GIFT_TOTAL } from "@/lib/wandsGiftBoard";
...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(count) || count < 1 || count > WANDS_GIFT_TOTAL) {
    return new Response("Bad request", { status: 400 });
  }

  const [slugs, status] = await Promise.all([
    pickNextUnusedWandsGift(count),
    pinterestPoolStatus(WANDS_GIFT_BOARD, WANDS_GIFT_TOTAL),
  ]);
