...
import { pickNextUnusedMajorsReclaimed, MAJOR_RECLAIMED_BOARD, MAJOR_RECLAIMED_TOTAL } from "@/lib/majorReclaimedBoard";
...
export async function GET(request: Request) {
  ...
  const [slugs, status] = await Promise.all([
    pickNextUnusedMajorsReclaimed(count),
    pinterestPoolStatus(MAJOR_RECLAIMED_BOARD, MAJOR_RECLAIMED_TOTAL),
  ]);
