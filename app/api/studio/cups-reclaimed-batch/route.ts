// Generates the ZIP for an EXACT set of Cups slugs (already previewed client-side) and, on
...
import { CUPS_RECLAIMED_BOARD } from "@/lib/cupsReclaimedBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Cups Reclaimed Reversal Meanings";
...
export async function GET(request: Request) {
  ...
  const cards = slugs
    .map(getCardBySlug)
    .filter((c): c is NonNullable<typeof c> => !!c && c.arcana === "minor" && c.meta.suit === "Cups");
  ...
  for (const card of cards) {
    const filename = `${card.slug}_cups-reclaimed.png`;
    const copy = suitReclaimedCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${CUPS_RECLAIMED_BOARD}/${card.slug}`;
    const text = [
  ...
  await markPinterestUsed(CUPS_RECLAIMED_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-cups-reclaimed_${cards.length}pins.zip"`,
    },
  });
}
