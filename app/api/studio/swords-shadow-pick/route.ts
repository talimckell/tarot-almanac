...
import { pickNextUnusedSwordsShadow, SWORDS_SHADOW_BOARD, SWORDS_SHADOW_TOTAL } from "@/lib/swordsShadowBoard";
...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(count) || count < 1 || count > SWORDS_SHADOW_TOTAL) {
    return new Response("Bad request", { status: 400 });
  }

  const [slugs, status] = await Promise.all([
    pickNextUnusedSwordsShadow(count),
    pinterestPoolStatus(SWORDS_SHADOW_BOARD, SWORDS_SHADOW_TOTAL),
  ]);
