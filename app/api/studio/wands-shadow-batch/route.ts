...
import { WANDS_SHADOW_BOARD } from "@/lib/wandsShadowBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Wands Shadow Meanings";
...
export async function GET(request: Request) {
  ...
  for (const card of cards) {
    const filename = `${card.slug}_wands-shadow.png`;
    const copy = suitShadowCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${WANDS_SHADOW_BOARD}/${card.slug}`;
    const text = [
  ...
  await markPinterestUsed(WANDS_SHADOW_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-wands-shadow_${cards.length}pins.zip"`,
    },
  });
}
