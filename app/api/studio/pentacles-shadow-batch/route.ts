...
import { PENTACLES_SHADOW_BOARD } from "@/lib/pentaclesShadowBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Pentacles Shadow Meanings";
...
export async function GET(request: Request) {
  ...
  for (const card of cards) {
    const filename = `${card.slug}_pentacles-shadow.png`;
    const copy = suitShadowCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${PENTACLES_SHADOW_BOARD}/${card.slug}`;
    const text = [
  ...
  await markPinterestUsed(PENTACLES_SHADOW_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-pentacles-shadow_${cards.length}pins.zip"`,
    },
  });
}
