...
import { SWORDS_GIFT_BOARD } from "@/lib/swordsGiftBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Swords Gift Meanings";
...
export async function GET(request: Request) {
  ...
  for (const card of cards) {
    const filename = `${card.slug}_swords-gift.png`;
    const copy = suitGiftCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${SWORDS_GIFT_BOARD}/${card.slug}`;
    const text = [
  ...
  await markPinterestUsed(SWORDS_GIFT_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-swords-gift_${cards.length}pins.zip"`,
    },
  });
}
