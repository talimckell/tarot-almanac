// Generates the ZIP for an EXACT set of Wands slugs (already previewed client-side) and, on
// success, marks them used for this board. Owner-only.
import { ImageResponse } from "next/og";
import JSZip from "jszip";
import { getCardBySlug } from "@/lib/cards";
import { requireStudioOwner } from "@/lib/studioAuth";
import { suitReclaimedCopy } from "@/lib/suitReclaimedCaption";
...
import { WANDS_RECLAIMED_BOARD } from "@/lib/wandsReclaimedBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Wands Reclaimed Reversal Meanings";
...
export async function GET(request: Request) {
  ...
  const cards = slugs
    .map(getCardBySlug)
    .filter((c): c is NonNullable<typeof c> => !!c && c.arcana === "minor" && c.meta.suit === "Wands");
  ...
  for (const card of cards) {
    const filename = `${card.slug}_wands-reclaimed.png`;
    const copy = suitReclaimedCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${WANDS_RECLAIMED_BOARD}/${card.slug}`;
    const text = [
    ...
    const image = new ImageResponse(renderSuitReclaimed(card), { width: WIDTH, height: HEIGHT, fonts });
  ...
  await markPinterestUsed(WANDS_RECLAIMED_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-wands-reclaimed_${cards.length}pins.zip"`,
    },
  });
}
