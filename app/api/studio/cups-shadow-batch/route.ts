...
import { CUPS_SHADOW_BOARD } from "@/lib/cupsShadowBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Cups Shadow Meanings";
...
export async function GET(request: Request) {
  ...
  for (const card of cards) {
    const filename = `${card.slug}_cups-shadow.png`;
    const copy = suitShadowCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${CUPS_SHADOW_BOARD}/${card.slug}`;
    const text = [
  ...
  await markPinterestUsed(CUPS_SHADOW_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-cups-shadow_${cards.length}pins.zip"`,
    },
  });
}
