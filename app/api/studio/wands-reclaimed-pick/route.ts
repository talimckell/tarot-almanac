...
import { pickNextUnusedWandsReclaimed, WANDS_RECLAIMED_BOARD, WANDS_RECLAIMED_TOTAL } from "@/lib/wandsReclaimedBoard";
...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(count) || count < 1 || count > WANDS_RECLAIMED_TOTAL) {
    return new Response("Bad request", { status: 400 });
  }

  const [slugs, status] = await Promise.all([
    pickNextUnusedWandsReclaimed(count),
    pinterestPoolStatus(WANDS_RECLAIMED_BOARD, WANDS_RECLAIMED_TOTAL),
  ]);
