...
import { SWORDS_RECLAIMED_BOARD } from "@/lib/swordsReclaimedBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Swords Reclaimed Reversal Meanings";
...
export async function GET(request: Request) {
  ...
  for (const card of cards) {
    const filename = `${card.slug}_swords-reclaimed.png`;
    const copy = suitReclaimedCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${SWORDS_RECLAIMED_BOARD}/${card.slug}`;
    const text = [
  ...
  await markPinterestUsed(SWORDS_RECLAIMED_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-swords-reclaimed_${cards.length}pins.zip"`,
    },
  });
}
