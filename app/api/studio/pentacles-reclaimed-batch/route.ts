...
import { PENTACLES_RECLAIMED_BOARD } from "@/lib/pentaclesReclaimedBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Pentacles Reclaimed Reversal Meanings";
...
export async function GET(request: Request) {
  ...
  for (const card of cards) {
    const filename = `${card.slug}_pentacles-reclaimed.png`;
    const copy = suitReclaimedCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${PENTACLES_RECLAIMED_BOARD}/${card.slug}`;
    const text = [
  ...
  await markPinterestUsed(PENTACLES_RECLAIMED_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-pentacles-reclaimed_${cards.length}pins.zip"`,
    },
  });
}
