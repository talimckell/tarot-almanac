...
import { CUPS_GIFT_BOARD } from "@/lib/cupsGiftBoard";
import { SITE_URL } from "@/lib/site";

export const runtime = "nodejs";

const BOARD_NAME = "Tarot Minor Arcana Suit of Cups Gift Meanings";
...
export async function GET(request: Request) {
  ...
  for (const card of cards) {
    const filename = `${card.slug}_cups-gift.png`;
    const copy = suitGiftCopy(card, BOARD_NAME);
    const mediaUrl = `${SITE_URL}/pin/${CUPS_GIFT_BOARD}/${card.slug}`;
    const text = [
  ...
  await markPinterestUsed(CUPS_GIFT_BOARD, cards.map((c) => c.slug));

  return new Response(new Uint8Array(zipBuffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="pinterest-cups-gift_${cards.length}pins.zip"`,
    },
  });
}
