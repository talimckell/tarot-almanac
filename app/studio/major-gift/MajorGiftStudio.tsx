...
export default function MajorGiftStudio({ initialStatus }: { initialStatus: PoolStatus }) {
  ...
  const generate = async () => {
    setBusy(true);
    try {
      const res = await fetch(`/api/studio/major-gift-pick?count=${count}`);
  ...
  // Awaiting the full response first removes any timing to get wrong.
  const download = async () => {
    if (!batch || batch.length === 0) return;
    setDownloading(true);
    try {
      const res = await fetch(`/api/studio/major-gift-batch?slugs=${batch.join(",")}`);
      const blob = await res.blob();
      const disposition = res.headers.get("content-disposition");
      const match = disposition ? /filename="([^"]+)"/.exec(disposition) : null;
      const filename = match ? match[1] : "pinterest-major-gift.zip";
      ...
      const statusRes = await fetch("/api/studio/major-gift-pick?count=1");
  ...
  const reset = async () => {
    if (!confirm("Reset the Major Gift board's usage tracking? All 22 Majors become eligible again.")) return;
    setResetting(true);
    try {
      const res = await fetch("/api/studio/major-gift-reset", { method: "POST" });
      const data = await res.json();
      setStatus(data.status);
      setBatch(null);
    } finally {
        ...
        >
          {batch.map((slug) => (
            <div key={slug} style={{ border: "1px solid var(--warm-stone)", borderRadius: 4, overflow: "hidden" }}>
              ...
              <img
                src={`/studio/major-gift/preview?slug=${slug}`}
