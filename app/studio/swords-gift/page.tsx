...
export const metadata: Metadata = {
  title: "Pinterest: Swords Gift Meanings Board Studio",
  robots: { index: false },
};

export default async function SwordsGiftStudioPage() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) redirect("/sign-in?next=/studio/swords-gift");
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <h1 style={{ fontFamily: "var(--serif)", fontSize: 32, margin: "6px 0 8px" }}>Suit of Swords Gift Meanings Board Studio</h1>
        <p style={{ color: "var(--label)", maxWidth: 640 }}>
          Batch-generate Pinterest pins for the &ldquo;Tarot Minor Arcana Suit of Swords Gift
          Meanings&rdquo; board &mdash; one pin per rank, upright/gift meaning, 14 total.
          Same bright saturated-field treatment as the other Gift boards.
        </p>
        <StudioNav except="/studio/swords-gift" />
        <SuitBoardStudio
          boardSlug={SWORDS_GIFT_BOARD}
          boardLabel="Swords Gift"
          total={SWORDS_GIFT_TOTAL}
