...
export default function MajorShadowStudio({ initialStatus }: { initialStatus: PoolStatus }) {
  ...
  const generate = async () => {
    setBusy(true);
    try {
      const res = await fetch(`/api/studio/major-shadow-pick?count=${count}`);
  ...
  const download = async () => {
    ...
    setDownloading(true);
    try {
      const res = await fetch(`/api/studio/major-shadow-batch?slugs=${batch.join(",")}`);
      const blob = await res.blob();
      const disposition = res.headers.get("content-disposition");
      const match = disposition ? /filename="([^"]+)"/.exec(disposition) : null;
      const filename = match ? match[1] : "pinterest-major-shadow.zip";
      ...
      const statusRes = await fetch("/api/studio/major-shadow-pick?count=1");
  ...
  const reset = async () => {
    if (!confirm("Reset the Major Shadow board's usage tracking? All 22 Majors become eligible again.")) return;
    setResetting(true);
    try {
      const res = await fetch("/api/studio/major-shadow-reset", { method: "POST" });
      const data = await res.json();
      setStatus(data.status);
      setBatch(null);
    } finally {
        ...
        >
          {batch.map((slug) => (
            <div key={slug} style={{ border: "1px solid var(--warm-stone)", borderRadius: 4, overflow: "hidden" }}>
              ...
              <img
                src={`/studio/major-shadow/preview?slug=${slug}`}
