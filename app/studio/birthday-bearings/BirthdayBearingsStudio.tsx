...
export default function BirthdayBearingsStudio() {
  ...
  const zipHref = `/api/studio/birthday-bearings-batch?start=${start}&count=${count}`;
        ...
        >
          {dates.map((date) => (
            <div key={date} style={{ border: "1px solid var(--warm-stone)", borderRadius: 4, overflow: "hidden" }}>
              ...
              <img
                src={`/studio/birthday-bearings/preview?date=${date}`}
                alt={`Birthday Bearing for ${date}`}
                width={1200}
                height={630}
